...
import {
  TEXT_PROD_DELETE_TITLE,
  TEXT_PROD_DELETE_MESSAGE,
  TEXT_PROD_DELETE_CANCEL,
  TEXT_PROD_DELETE_CONFIRM,
} from "../../../../../../constants/i18nKeys";
import { IconAlertTriangle } from "../../../../../../constants/icons";

export default function DeleteProductModal({ visible, product, deleting, onClose, onConfirm }) {
  const { t } = useLocalization();

  if (!visible || !product) return null;

  const message = t(TEXT_PROD_DELETE_MESSAGE).replace("{{name}}", product.name);
